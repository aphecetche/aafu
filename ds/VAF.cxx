...
//______________________________________________________________________________
TString VAF::CSS() const
{
  TString css = "<link rel=\"stylesheet\" type=\"text/css\" href=\"af.css\"/>\n";
  return css;
}
...
//______________________________________________________________________________
void VAF::GenerateHTMLDataRepartition(const TMap& groups)
{
  TString table;
  ...
  {
    keys.Add(new TObjString(s->String()));
  }
  ...
  while ( ( s = static_cast<TObjString*>(nextKey()) ) )
  {
    if ( !s->String().BeginsWith("SERVER") ) continue;
    ...
    a = value.Tokenize(".");
    TString server =static_cast<TObjString*>(a->First())->String();
    delete a;
    
    table += Form("[ '%s', { v:%7.2f, f:'%7.2f GB'} ],\n",server.Data(),size/byte2GB,size/byte2GB);
    ULong64_t fileSize = GenerateASCIIFileList(key.Data(),server.Data(),*list);
    
    TString filename;
    
    filename.Form("%s.%s.%s.txt",fMaster.Data(),key.Data(),server.Data());
  ...
  js += Form("  var filename = '%s.SERVER.' + data.getValue(selectedItem.row,0) + '.txt';\n",
             fMaster.Data());
  ...
  html += HTMLHeader(fMaster.Data(),css,js);
  
  html += Form("<div><h1>Disk space usage repartition on %s</h1></div>\n",fMaster.Data());
  ...
  std::ofstream out(Form("%s/%s",fHtmlDir.Data(),FileNameDataRepartition().Data()));
  out << html.Data();
  
}

//______________________________________________________________________________
void VAF::GenerateHTMLDatasetList(const TMap& groups)
{
  TObjArray keys;
  ...
  std::ofstream outfile(Form("%s/%s",fHtmlDir.Data(),FileNameDataSetList().Data()));
  ...
  while ( ( s = static_cast<TObjString*>(nextSortedGroup()) ) )
  {
    ...
    assert (list!=0);
    ULong64_t size = SumSize(*list);
    lines += Form("<tr><td>%-50s</td><td>%6d</td><td>%7.2f</td></tr>\n",s->String().Data(),list->GetSize(), size/byte2GB);
  }
...
//______________________________________________________________________________
void VAF::GenerateHTMLPieCharts(const TMap& groups)
{
  ...
  for ( std::map<std::string,std::string>::const_iterator it = tables.begin(); it != tables.end(); ++it )
  {
    ...
    js += Form("var data%s = google.visualization.arrayToDataTable([\n",it->first.c_str());
    js += it->second.c_str();
    js += "]);\n";
    js += Form("var options%s = { \n",it->first.c_str());
    js += Form("title: 'Disk space by %s',\n",it->first.c_str());
    js += "pieSliceText: 'label' };\n";
    js += Form("var chart%s = new google.visualization.PieChart(document.getElementById('piechart_%s'));\n",
               it->first.c_str(),it->first.c_str());
    js += Form("chart%s.draw(data%s,options%s);\n",it->first.c_str(),it->first.c_str(),it->first.c_str());
    
    js += Form("function selectHandler%s() {\n",it->first.c_str());
    js += Form("var selectedItem = chart%s.getSelection()[0];\n",it->first.c_str());
    js += "if (selectedItem) {\n";
    js += Form("  var sel = data%s.getValue(selectedItem.row,0);\n",it->first.c_str());
    
    js += Form("  var filename = '%s.%s.' + data%s.getValue(selectedItem.row,0) + '.txt';\n",
    ...
    js += Form("google.visualization.events.addListener(chart%s,'select',selectHandler%s);\n",
               it->first.c_str(),it->first.c_str());
  }
  ...
  std::ofstream out(Form("%s/%s",fHtmlDir.Data(),FileNamePieCharts().Data()));
  out << html.Data();
...
void VAF::GenerateHTMLReports(const TList& fileInfoList)
{
  ...
  std::cout << "Generating dataset list..." << std::endl;
  GenerateHTMLDatasetList(g);  
  std::cout << "done." << std::endl;
  
  std::cout << "Generating pie charts..." << std::endl;
  // Make the pie charts
  GenerateHTMLPieCharts(g);
  std::cout << "done." << std::endl;

  std::cout << "Generating data repartition..." << std::endl;
  GenerateHTMLDataRepartition(g);
  std::cout << "done." << std::endl;
  ...
  TString html = HTMLHeader(fMaster.Data(),CSS(),"");
  
  html += Form("<h1>%s</h1>\n",fMaster.Data());
  
  html += "<nav class=\"mainmenu\">\n";

  html += Form("<a href=\"%s.usermanual.html\">User manual</a>\n",fMaster.Data());

  html += Form("<a href=\"%s\">Pie charts of disk usage by file type, data type, pass, etc... </a>\n",FileNamePieCharts().Data());

  html += Form("<a href=\"%s\">Tree map of disk usage</a>\n",FileNameTreeMap().Data());

  html += Form("<a href=\"%s\">Raw list of dataset groups</a>\n",FileNameDataSetList().Data());

  html += Form("<a href=\"%s\">Data repartition by server</a>\n",FileNameDataRepartition().Data());
  ...
  // copy a few files to the HtmlDir
  std::vector<std::string> filesToBeCopied;
  
  filesToBeCopied.push_back("af.css");
  filesToBeCopied.push_back("jquery.tocify.min.js");
  filesToBeCopied.push_back(Form("%s.usermanual.md",fMaster.Data()));
  
  std::vector<std::string> filesToBeRemoved;
  
  for ( std::vector<std::string>::size_type i = 0; i < filesToBeCopied.size(); ++i )
  {
    if ( gSystem->AccessPathName(filesToBeCopied[i].c_str()) == kFALSE)
    {
      TString outfile = filesToBeCopied[i].c_str();
      
      if ( outfile.EndsWith(".md") )
      {
        if ( gSystem->Exec("which pandoc") )
        {
        ...
        TString tmpFile("fragment.html");
        
        if (gSystem->Exec(Form("pandoc %s -o %s",outfile.Data(),tmpFile.Data()))) continue;
        
        std::ifstream in(tmpFile.Data());
        
        TString fragment;
        TString html;
        
        fragment.ReadFile(in);
        
        TString css = CSS();
        TString js = JSTOC();
        
        html += HTMLHeader(Form("%s User Manual",fMaster.Data()),css,js);
        
        html += "<div id=\"toc\"></div>\n";
        
        html += fragment.Data();
        
        html += HTMLFooter(kTRUE);
        
        outfile.ReplaceAll(".md",".html");
        
        std::ofstream out(outfile);
        out << html.Data() << std::endl;
        out.close();
       
        filesToBeRemoved.push_back(tmpFile.Data());
        filesToBeRemoved.push_back(outfile.Data());
        
      }
      
      gSystem->CopyFile(outfile.Data(),Form("%s/%s",fHtmlDir.Data(),outfile.Data()),kTRUE);
    }
  }
  
  for ( std::vector<std::string>::size_type i = 0; i < filesToBeRemoved.size(); ++i )
  {
    gSystem->Unlink(filesToBeRemoved[i].c_str());
  }
}
...
void VAF::GenerateHTMLTreeMap(const TList& fileInfoList)
{
  ...
  TString html;
  TString css = CSS();
  TString js = JSGoogleChart("treemap");
  ...
  std::ofstream out(Form("%s/%s",fHtmlDir.Data(),FileNameTreeMap().Data()));
  out << html.Data();
...
void VAF::GroupFileInfoList(const TList& fileInfoList, TMap& groups)
{
  ...
  while ( ( fileInfo = static_cast<AFFileInfo*>(next()) ) )
  {
    ...
    // by server
    AddFileToGroup(groups,Form("SERVER:%s",fileInfo->fHostName.Data()),*fileInfo);
...
//______________________________________________________________________________
TString VAF::HTMLFooter(Bool_t withJS) const
{
  TString footer;
...
//______________________________________________________________________________
TString VAF::HTMLHeader(const char* title, const TString& css, const TString& js) const
{
  TString header;
...
//______________________________________________________________________________
TString VAF::JSGoogleChart(const char* chartPackage) const
{
  TString js;
  ...
  js += Form("google.load(\"visualization\", \"1\", {packages:[\"%s\"]});\n",chartPackage);
...
//______________________________________________________________________________
TString VAF::JSListJumper() const
{
  TString js;
...
//______________________________________________________________________________
TString VAF::OutputHtmlFileName(const char* type) const
{
  TString name;
  
  name.Form("%s.%s.html",fMaster.Data(),type);
